...
import '../../marketplace/providers/food_provider.dart';
import '../../marketplace/models/food_item_model.dart';
import '../../../shared/utils/price_utils.dart';
import '../../marketplace/screens/merchant_details_screen.dart';
...
class _HomeContentScreenState extends State<HomeContentScreen> {
  ...
  void _navigateToCategory(dynamic categoryOrTag, {bool isDietaryTag = false}) {
    if (isDietaryTag) {
      // Navigate to category page with dietary tag filter
      final tagName = categoryOrTag.toString().split('.').last.toLowerCase();
      context.push('/category/$tagName');
    } else {
      // Navigate to category page using FoodCategory enum name
      final category = categoryOrTag as FoodCategory;
  ...
  Widget _buildCategoriesSection() {
    final categories = [
      {'label': 'Mystery Bag', 'category': FoodCategory.mysteryBag, 'icon': Icons.card_giftcard, 'isDietaryTag': false},
      {'label': 'Meals', 'category': FoodCategory.preparedMeals, 'icon': Icons.lunch_dining, 'isDietaryTag': false},
      {'label': 'Bakery', 'category': FoodCategory.bakery, 'icon': Icons.bakery_dining, 'isDietaryTag': false},
      {'label': 'Beverages', 'category': FoodCategory.beverages, 'icon': Icons.local_drink_outlined, 'isDietaryTag': false},
      {'label': 'Vegetarian', 'category': DietaryTag.vegetarian, 'icon': Icons.eco, 'isDietaryTag': true},
      {'label': 'Halal', 'category': DietaryTag.halal, 'icon': Icons.verified, 'isDietaryTag': true},
    ];
              ...
              child: Transform.translate(
                ...
                child: GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final dynamic categoryOrTag = c['category'];
                    final bool isDietaryTag = c['isDietaryTag'] as bool;
                    final bool isSelected = isDietaryTag 
                        ? foodProvider.selectedDietaryTags.contains(categoryOrTag)
                        : foodProvider.selectedCategories.contains(categoryOrTag);
                    
                    return GestureDetector(
                      onTap: () => _navigateToCategory(categoryOrTag, isDietaryTag: isDietaryTag),
  ...
  Widget _buildSurplusCard(FoodItemModel item) {
    ...
    return Card(
      ...
      child: InkWell(
              onTap: () {
                context.push('/merchant/${item.merchantId}');
              },
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
